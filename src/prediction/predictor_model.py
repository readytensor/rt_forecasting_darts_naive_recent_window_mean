...
class Forecaster:
    ...
    model_name = "Naive Recent Window Mean Forecaster"

    def __init__(
        self,
        data_schema: ForecastingSchema,
        history_forecast_ratio: int = 1,
        random_state: int = 0,
        **kwargs,
    ):
        """Construct a new Naive Recent Window Mean Forecaster

        For a forecast window length `f`, this mean uses the mean of the latest `f` points
        in history. This mean value is the forecasted value for all `f` time steps in the
        forecast window. 
